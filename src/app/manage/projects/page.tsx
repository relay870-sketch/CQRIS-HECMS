        ...
        {projects.length === 0 ? <div className="rounded-2xl bg-white py-16 text-center text-gray-400">暂无项目</div> : (
          <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
            <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-left text-sm">
              <thead className="bg-gray-50 text-xs text-gray-500"><tr><th className="px-5 py-3">项目名称</th><th className="px-4 py-3">标段</th><th className="w-24 whitespace-nowrap px-4 py-3 text-center">状态</th><th className="px-4 py-3">负责人</th><th className="px-4 py-3">工期</th><th className="px-4 py-3">进度</th><th className="px-5 py-3 text-right">操作</th></tr></thead>
              <tbody className="divide-y divide-gray-100">{projects.map((project) => <tr key={project.id} className="hover:bg-blue-50/30">
                ...
                <td className="w-24 whitespace-nowrap px-4 py-4 text-center"><span className={`inline-flex whitespace-nowrap rounded-full px-2.5 py-1 text-xs ${getStatusColor(project.status)}`}>{getStatusLabel(project.status)}</span></td>
